<?php

@include 'config.php';

session_start();

$user_id = $_SESSION['user_id'];

if(!isset($user_id)){
   header('location:login.php');
};

?>

<!DOCTYPE html>
<html lang="en">
<head>
   <meta charset="UTF-8">
   <meta http-equiv="X-UA-Compatible" content="IE=edge">
   <meta name="viewport" content="width=device-width, initial-scale=1.0">
   <title>orders</title>

   <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.1/css/all.min.css">

   <link rel="stylesheet" href="css/web.css">

</head>
<body>
   
<?php include 'header.php'; ?>

<section class="place-order">

   <h1 class="title">Daftar Order</h1>

   <div class="box-container">

      <?php
         $select_orders = $conn->prepare("SELECT * FROM `orders` WHERE user_id = ?");
         $select_orders->execute([$user_id]);
         if($select_orders->rowCount() > 0){
            while($fetch_orders = $select_orders->fetch(PDO::FETCH_ASSOC)){    
      ?>
      <div class="box">
         <p>nama : <span><?= $fetch_orders['name']; ?></span></p>
         <p>nomor : <span><?= $fetch_orders['number']; ?></span></p>
         <p>email : <span><?= $fetch_orders['email']; ?></span></p>
         <p>address : <span><?= $fetch_orders['address']; ?></span></p>
         <p>pembayaran : <span><?= $fetch_orders['method']; ?></span></p>
         <p>order : <span><?= $fetch_orders['total_products']; ?></span></p>
         <p>total : <span>Rp. <?= number_format($fetch_orders['total_price'], 0, ',', '.'); ?></span></p>
         <p>date : <span><?= $fetch_orders['placed_on']; ?></span></p>
         <p>status pembayaran : <span><?= $fetch_orders['payment_status']; ?></span></p>
      </div>
      <?php
         }
      } else {
            '<p class="empty">order anda kosong</p>';
         }
      ?>

   </div>

</section>

<?php include 'footer.php'; ?>

<script src="js/script.js"></script>

</body>
</html>